...
const fn_one = async (m_id) => {
  console.log("fn_one() / m_id => ", m_id);
  console.log("fn_one() / pm_id => ", e.target.dataset.m_id);
...
<template>
  ...
  <table>
    ...
    <tr v-for="item in list" :key="item.m_id">
      <td @click.prevent="fn_one" v-bind:data-m_id="item.m_id">{{ item.m_id }}</td>
      <td>{{ item.m_name }}</td>
      <td>{{ item.tel }}</td>
      <td>{{ item.email }}</td>
    </tr>
